"""remove task table

Revision ID: dd1d6055aa1c
Revises: 4d76feed4882
Create Date: 2021-02-01 11:27:43.960429

"""
from alembic import op
import sqlalchemy as sa
from sqlalchemy.dialects import mysql

# revision identifiers, used by Alembic.
revision = 'dd1d6055aa1c'
down_revision = '4d76feed4882'
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index('ix_task_name', table_name='task')
    op.drop_table('task')
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('task',
    sa.Column('id', mysql.VARCHAR(collation='utf8_bin', length=36), nullable=False),
    sa.Column('name', mysql.VARCHAR(collation='utf8_bin', length=128), nullable=True),
    sa.Column('description', mysql.VARCHAR(collation='utf8_bin', length=128), nullable=True),
    sa.Column('user_id', mysql.INTEGER(), autoincrement=False, nullable=True),
    sa.Column('complete', mysql.TINYINT(display_width=1), autoincrement=False, nullable=True),
    sa.CheckConstraint('(`complete` in (0,1))', name='task_chk_1'),
    sa.ForeignKeyConstraint(['user_id'], ['reservation.id'], name='task_ibfk_1'),
    sa.PrimaryKeyConstraint('id'),
    mysql_collate='utf8_bin',
    mysql_default_charset='utf8',
    mysql_engine='InnoDB'
    )
    op.create_index('ix_task_name', 'task', ['name'], unique=False)
    # ### end Alembic commands ###
